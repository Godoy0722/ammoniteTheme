...
class AmmoniteThemePlugin extends ThemePlugin
{
    ...
    public function siteWideData(string $hookName, array $args)
    {
        ...
        $templateMgr->assign([
            'siteWideDisplayPageHeaderLogo' => $context->getLocalizedData('pageHeaderLogoImage'),
            ...
            'footerClientLogo' => $router->url($request, null, 'libraryFiles', 'downloadPublic', $this->getOption('footerClientLogo')),
            'defaultIssueCoverImg' => $router->url($request, null, 'libraryFiles', 'downloadPublic', $this->getOption('issueDefaultCoverImage')),
            'defaultArticleImage' => $router->url($request, null, 'libraryFiles', 'downloadPublic', $this->getOption('defaultArticleImage')),
            'primaryColour' => $this->getOption('baseColour'),
            'articleMetadataToShow' => $this->getOption('articleMetadataToShow'),
        ]);
    }
    ...
    public function hasEmbeddedCSS(string $hookName, array $args)
    {
        ...
        foreach ($articleArrays->getGalleys() as $galley) {
            if ($galley->getFileType() === 'text/html') {
                ...
                $embeddableFiles = [
                    ...$submissionFileDao->getLatestRevisions($submissionFile->getSubmissionId()),
                    ...$submissionFileDao->getLatestRevisionsByAssocId($submissionFile->getFileId(), $submissionFile->getSubmissionId())
                ];
